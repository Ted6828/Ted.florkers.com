...
var Essential_Audio = (() => {
    ...
    var p;
    var q = 50;
    var essential_audio_div = "div.essential_audio";
    ...
    function onReadyFunction() {
        // console.log(`pb: ${pb}`);
        var ia = document.querySelectorAll(essential_audio_div);
        ia.forEach((playerButton, ib) => {
            playerButton.innerHTML = "<div><div class=\"off\"><!----></div></div><div><div><!----></div></div><div><!----></div>";
            var playerButtonId;
            if (playerButton.hasAttribute("id")) {
                playerButtonId = playerButton.getAttribute("id")
            } else {
                playerButtonId = ib + 1;
                playerButton.setAttribute("id", playerButtonId)
            }
            // console.log(playerButton)
            // console.log(playerButtonId)
            // create element audio with ID "audio-player"
            var essentialAudioDiv = document.getElementsByClassName("essential_audio")[0]
            A[playerButtonId] = document.createElement("audio", {'id': 'audio-player'});
            A[playerButtonId].volume = 0.1; // RIGHT HERE CHANGE THE VOLUME HERE TED !!!! (setup by yi5 btw)
            var a = A[playerButtonId];
            // console.log(a)
            a.id = playerButtonId;
            a.za = playerButton.querySelector(pb).querySelector("div");
            a.zb = a.za.offsetWidth;
            a.zc = playerButton.querySelector(pb).offsetWidth - a.zb;
            if (a.zc < 0) {
                a.zc = 0
            }
            // ----------------- THIS IS THE LINE THAT CHOOSES THE SONG -----------------
            var songs = [
                'Eminem_Stan',
                'In_This_Shirt',
            ...
            function randomSong() {
                var i = parseInt(Math.random() * songs.length);
                console.log("Playing " + songs[i] + ".mp3")
                return "assets/audio/" + songs[i] + ".mp3";
            }
            a.zd = randomSong(); // THIS IS THE LINE THAT CHOOSES THE SONG TED ACTUALLY FOUND THIS BIT EASY DUBS!
            // --------------------------------------------------------------------------
            a.ze = 0;
            a.zf = 0;
            a.zh = 0;
            ...
            a.preload = "metadata";
            if (playerButton.hasAttribute("data-loop")) {
                a.loop = t
            }
            if (playerButton.hasAttribute("data-scratch")) {
                a.zr = t
            } else {
                a.zr = f
            }
            if (playerButton.hasAttribute("data-autoplay")) {
                if (h < 1) {
                    h = 1;
                    a.autoplay = t;
                    a.preload = "auto";
            ...
            if (playerButton.hasAttribute("data-preload")) {
                if (!a.autoplay) {
                    a.preload = "auto";
                    E(a)
                }
            ...
            var ie = playerButton.querySelector("div:nth-child(3)");
            ie.onmousedown = (e) => {
                e.stopPropagation();
                e.preventDefault();
                Bb(a)
    ...
    function W() {
        var vm = document.querySelectorAll(essential_audio_div);
        vm.forEach((vr) => {
